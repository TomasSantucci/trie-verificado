...
let rec insert (t : trie0 #alphabet_size) (l: list b_nat) : Tot trie0 (decreases l) =
  match (t,l) with
  | (L, []) -> N (empty_array, true, 0)
  | (N (a, _, n), []) -> N (a, true, n)
  | (L, x::xs) -> N (init alphabet_size (fun i -> if i = x then insert L xs else L), false, 1)
  | (N (a, b, n), x::xs) ->
    match index a (U32.uint_to_t x) with
    | L -> N (update a (U32.uint_to_t x) (insert L xs), b, n + 1)
    | t' -> N (update a (U32.uint_to_t x) (insert t' xs), b, n)

let rec delete (t : trie0 #alphabet_size) (l: list b_nat) : Tot (trie0 #alphabet_size) (decreases l) = 
  match (t,l) with
  | (L, _) -> L
  | (N (_, _, 0), []) -> L
  | (N (a, _, n), []) -> N (a, false, n)
